...
//========================Admin Index=============================================
	public void adminIndex(String userId) {
		...
		case 6:
			...
			else adminControler.deleteCourse(courseId);
			break;
		case 7:
...
//==========================printing trainers info=================================
	...
	public void showTrainerInfo(List<TrainerCredentials> trainerInfo) {
		System.out.printf("%15s |%15s |%15s |%15s |%15s |%20s  |%n","UserId","name","Date-Of-Birth","Experience","PhoneNo","mailId");	
		for(TrainerCredentials val:trainerInfo)
		{
			System.out.printf("%15s |%15s |%15s |%15d |%15s | %20s |%n",val.getUserId(),val.getName(),val.getDob().toString(),val.getExperience(),val.getPhoneNo(),val.getMailId());
		}
		checkContinue();
	}
	...
	public void chooseTrainers(List<TrainerCredentials> trainerInfo) {
		System.out.printf("%5s |%15s |%15s |%15s |%15s |%15s | %n","UserId","name","Date-Of-Birth","Experience","PhoneNo","mailId");	
		for(TrainerCredentials val:trainerInfo)
		{
			System.out.printf("%5s |%15s |%15s |%15d |%15s |%15s | %n",val.getUserId(),val.getName(),val.getDob().toString(),val.getExperience(),val.getPhoneNo(),val.getMailId() );
		}	
	}
}
